...
function filterModelsByKeys(keys: Record<string, string>) {
  return ALL_MODELS.filter(m => !!keys[m.provider]);
}
  ...
  const html = `<!DOCTYPE html><html><head>
    ...
    <style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      h1 { font-size: 22px; font-weight: 700; margin-bottom: 6px; color: #0f172a; }
      .meta { font-size: 12px; color: #64748b; margin-bottom: 36px; }
      .message { margin-bottom: 24px; display: flex; gap: 14px; }
      .avatar { width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 700; flex-shrink: 0; margin-top: 2px; }
      .avatar.user { background: #2563eb; color: #fff; }
      .avatar.assistant { background: #334155; color: #e2e8f0; }
      .bubble { flex: 1; }
      .role { font-size: 11px; font-weight: 700; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.06em; }
      .role.user-role { color: #2563eb; }
      .role.ai-role { color: #7c3aed; }
      .content { font-size: 14px; line-height: 1.7; white-space: pre-wrap; word-break: break-word; padding: 14px 18px; border-radius: 14px; }
      .user-content { background: #eff6ff; color: #1e3a8a; }
      .ai-content { background: #f8fafc; border: 1px solid #e2e8f0; color: #1e293b; }
      .time { font-size: 10px; color: #94a3b8; margin-top: 6px; }
      hr { border: none; border-top: 1px solid #e2e8f0; margin: 32px 0; }
      @media print { body { padding: 20px; } .no-print { display: none; } }
    </style>
  </head><body>
    <h1>${title}</h1>
    <p class="meta">📄 Exportálva: ${new Date().toLocaleString('hu-HU')} · ${messages.length} üzenet</p>
    <hr>
    ${messages.map(m => `
      <div class="message">
        <div class="avatar ${m.role}">${m.role === 'user' ? '👤' : '🤖'}</div>
        <div class="bubble">
          <div class="role ${m.role === 'user' ? 'user-role' : 'ai-role'}">${m.role === 'user' ? 'Te' : 'AI Vivien'}</div>
          <div class="content ${m.role === 'user' ? 'user-content' : 'ai-content'}">${m.content.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
          <div class="time">${new Date(m.created_at).toLocaleString('hu-HU')}</div>
        </div>
      </div>`).join('')}
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {!modelsLoaded ? (
                      ...
                      <div className="relative">
                        ...
                        {showModelMenu && (
                          <div className="absolute bottom-full left-0 mb-2 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden z-10 w-64">
                            <div className="p-1">
